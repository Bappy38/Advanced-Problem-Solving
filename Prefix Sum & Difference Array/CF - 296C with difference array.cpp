...
struct info
{
    ll l,r,x;
    info(){}
    info(ll _l,ll _r,ll _x)
    {
        l = _l; r = _r; x = _x;
    }
}query[100005];
ll ara[100005],qryara[100005],difara[100005],difqryara[100005];
...
void Solve(int t)
{
    ...
    for(i=1 ; i<=q ; i++){
        qryara[i] = difqryara[i]+qryara[i-1];
        l = query[i].l;
        r = query[i].r;
        x = qryara[i]*query[i].x;
        difara[l] += x;
        difara[r+1] -= x;
    }
    for(i=1 ; i<=n ; i++){
        ara[i] = difara[i]+ara[i-1];
        cout<<ara[i]<<' ';
    }
    cout<<endl;
}
