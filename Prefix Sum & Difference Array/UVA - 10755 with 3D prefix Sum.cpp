...
void Solve(int t)
{
    ll i,j,k,a,b,c,x1,y1,z1,x2,y2,z2,ans=-longlimit,q;
        ...
        for(y1=1 ; y1<=b ; y1++){
            for(z1=1 ; z1<=c ; z1++){
                for(x2=x1 ; x2<=a ; x2++){
                    for(y2=y1 ; y2<=b ; y2++){
                        for(z2=z1 ; z2<=c ; z2++){
                            ll temp = PrefixSum[x2][y2][z2] - PrefixSum[x1-1][y2][z2] - PrefixSum[x2][y1-1][z2] - PrefixSum[x2][y2][z1-1] + PrefixSum[x1-1][y1-1][z2] + PrefixSum[x2][y1-1][z1-1] + PrefixSum[x1-1][y2][z1-1] - PrefixSum[x1-1][y1-1][z1-1];
                            ans = max(ans , temp);
                        }
                    }
                }
