...
ll ara[100005],fg[100005],prefsum[100005],sum[100005];

void Solve(int t)
{
    ll i,j,k,n,mxprofit=0,ans=0;
    scln2(n , k);
    ...
    for(i=1 ; i<=n-k+1 ; i++){
        ll lb = i;
        ll ub = i+k-1;
        ll tot = sum[ub] - sum[lb-1];
        ll paici = prefsum[ub] - prefsum[lb-1];
        mxprofit = max(mxprofit , tot-paici);
    }
    ans = prefsum[n]+mxprofit;
    cout<<ans<<endl;
}
