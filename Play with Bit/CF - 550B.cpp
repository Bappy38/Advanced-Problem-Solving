...
void Solve(int t)
{
    int i,j,k,n,x,l,r,ans=0;
    cin>>n>>l>>r>>x;
    rep(i,0,n-1)scin(ara[i]);
    for(int mask=0 ; mask<=(1<<n)-1 ; mask++){
        int taken=0,mntake=infinity,mxtake=0,tot=0;
        for(i=0 ; i<n ; i++){
            if(Cheek(mask , i)){
                mntake = min(mntake , ara[i]);
                mxtake = max(mxtake , ara[i]);
                taken++;
                tot += ara[i];
            }
        }
        if(taken>=2 && tot>=l && tot<=r && mxtake-mntake>=x){
//            error(mask , taken , tot , mntake , mxtake);
            ans++;
        }
    }
    cout<<ans<<endl;
