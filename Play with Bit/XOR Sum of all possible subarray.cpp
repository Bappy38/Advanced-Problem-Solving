...
/** You're given an array with n elements. Calculate XOR sum of all possible subarray.
Like:
for(int l=1 ; l<=n ; l++){
    for(int r=l ; r<=n ; r++){
        int XOR = 0;
...
XOR sum of all possible SubArray = all pair XOR sum of prefixXOR array of that array.
**/

int ara[100005],prefix[100005],cnt[2][35];
...
void Solve(int t)
{
    ...
    for(i=n ; i>=0 ; i--){
        for(j=0 ; j<=30 ; j++){
            int jthbitofi = Cheek(prefix[i] , j);
            if(jthbitofi)
                ans += ((1<<j)*cnt[0][j]); ///cnt[x][j] - number of x in jth position for ara[i+1....n]
            else
                ans += ((1<<j)*cnt[1][j]);
            cnt[jthbitofi][j]++;
        }
