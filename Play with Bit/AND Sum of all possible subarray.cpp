...
/** Problem: You're given an array with n elements. You have to calculate AND sum
of all possible SubArray.Like:
for(int l=1 ; l<=n ; l++){
    for(int r=l ; r<=n ; r++){
        int AND = (1<<30)-1;
        for(int i=l ; i<=r ; i++
...
Solution:
1. IF there're x consecutive 1 in a bit position. That means there're (x*(x+1))/2
SubArray where this bit is contribute. Sum up (1<<pos)*((x*(x+1))/2) with ans.
**/
    ...
    for(int pos=0 ; pos<=30 ; pos++){
        ...
        for(int i=1 ; i<=n ; i++){
            if(Cheek(ara[i] , pos))
                ...
                int contribute= (conone * (conone+1))/2;
                ans += (contribute * (1<<pos));
                conone = 0;
            }
        }
        int contribute = (conone * (conone+1))/2;
