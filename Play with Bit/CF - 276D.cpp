...
ll Cheek(ll N, ll pos) {return  (bool)(N & (1LL<<pos));}
...
void Solve(int t)
{
    ll i,j,k,ans=0,l,r,a=0LL,b=0LL;
    cin>>l>>r;
    if(l == r){
        pf("0\n");
        return;
    }
    for(ll pos=60LL ; pos>=0LL ; pos--){
        ll thisval = 1LL<<pos;
        if(thisval > r)
            continue;
//        cout<<thisval<<' '<<a<<' '<<b<<endl;
        if(b+thisval-1 >= l && a+thisval<=r){
//            pf("HI1 - %lld\n",pos);
            a = Set(a , pos);
            ans += thisval;
        }
        else if(a+thisval-1 >= l && b+thisval<=r){
//            pf("HI2 - %lld\n",pos);
            b = Set(b , pos);
            ans += thisval;
        }
        else if(a+thisval<=r && b+thisval<=r){
//            pf("HI3 - %lld\n",pos);
            a = Set(a , pos);
            b = Set(b , pos);
        }
