...
/** You're given a binary array of n elements which contains only 0 & 1. Find all pair
XOR sum of the binary array.
Example: ara[n] = {1 , 0 , 1 , 0 , 1}
ans = (1+0+1+0) + (1+0+1) + (1+0) + (1) = 6
...
int ara[100005],cnt[2];

void Solve(int t)
{
    int i,j,k,one=0,zero=0,ans=0,n;
    scin(n);
    for(i=1 ; i<=n ; i++){
        scin(ara[i]);
        cnt[ara[i]]++;
